function svcbound(X,Y,ker,alpha,bias,aspect,mag,xaxis,yaxis,input)
...
%     for i = 1:size(Y)
%       if (Y(i) == 1)
%         plot(X(i,xaxis),X(i,yaxis),'ks','MarkerFaceColor',[.75 .75 .75]); % Class A
%          if (abs(alpha(i)) > epsilon)
%             plot(X(i,xaxis),X(i,yaxis),'ks','MarkerFaceColor','k') % Support Vector
%          end
%       else
%          plot(X(i,xaxis),X(i,yaxis),'ko','MarkerFaceColor','c'); % Class B
%          if (abs(alpha(i)) > epsilon)
%             plot(X(i,xaxis),X(i,yaxis),'ko','MarkerFaceColor','k') % Support Vector
%          end
%       end
%     end 
