...
I_transform = imtransform(I,Tform1,'bicubic','Xdata',[1 N],'Ydata',[1 M]);
%
...
for i = 1:2  % Get reference point for both images
    ...
    for j = 1:nu_points    
        ...
        plot(x(j,i), y(j,i),'X','MarkerSize',14,'MarkerEdgeColor','w');  % mark ref. point
    end
end    
%
[Tform2, inpts, base_pts] = cp2tform([x(:,2) y(:,2)],[x(:,1) y(:,1)],'projective');
I_aligned = imtransform(I_transform,Tform2,'bicubic','Xdata',[1 N],'Ydata',[1 M]);   
%
disp([inpts base_pts]);
...
imshow(I);       %Display the images
    ...
    for j = 1:nu_points    
          plot(x(j,1), y(j,1),'X','MarkerSize',14,'MarkerEdgeColor','w');  % mark ref. point
    end
subplot(1,3,2);  hold on
imshow(I_transform);
    for j = 1:nu_points    
        plot(x(j,2), y(j,2),'X','MarkerSize',14,'MarkerEdgeColor','w');  % mark ref. point
    end
